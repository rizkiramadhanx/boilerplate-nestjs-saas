...
export class AdminsBackofficeService {
  ...
  async findOne(id: string) {
    ...
    if (!admin) throw new NotFoundException(t('admin_not_found'));
  ...
  async create(dto: CreateAdminBackofficeDto) {
    ...
    if (existing) {
      throw new ConflictException(t('admin_email_already_exists'));
    }
  ...
  async register(dto: RegisterAdminBackofficeDto) {
    const secret = this.configService
      ...
      ?.trim();
    if (!secret) {
      throw new ForbiddenException(t('admin_registration_not_configured'));
    }
    if (dto.register_secret?.trim() !== secret) {
      throw new ForbiddenException(t('admin_register_secret_invalid'));
    }
    const existing = await this.adminRepository.findOne({
      where: { email: dto.email },
    });
  ...
  async login(dto: LoginAdminBackofficeDto) {
    ...
    if (!admin) {
      throw new UnauthorizedException(t('admin_invalid_email_or_password'));
    }
