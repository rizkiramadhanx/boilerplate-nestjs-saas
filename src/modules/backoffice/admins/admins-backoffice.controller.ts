...
export class AdminsBackofficeController {
  ...
  async registerAdmin(
    ...
    try {
      ...
      const message =
        err instanceof HttpException ? err.message : 'Failed to register admin';
      res.status(status);
  ...
  async loginAdmin(
    ...
    try {
      ...
      res.status(HttpStatus.OK);
      return createSuccessResponse(t('admin_login_success'), result);
    } catch (err) {
      ...
      const message =
        err instanceof HttpException
          ? err.message
          : t('admin_invalid_email_or_password');
      res.status(status);
      return createErrorResponse(message, status);
    }
  }
